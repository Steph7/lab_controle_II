...
% formato = 2 (invertido)
    ...
    if formato == 1
        % data | saida | controle | referencia
        valor_ref = v3;
        controle = v2;
        valor = v1;
    ...
    figure;
    subplot(2,1,1)
    plot(tempo_segundos, valor, 'r', 'LineWidth', 1)
    hold on
    plot(tempo_segundos, valor_ref, 'b', 'LineWidth', 1.2)
    ylabel("Nível do TP")
...
mf1 = readtable('dadosModelagem.txt'); % Todos os dados
mf2 = readtable('dados_parte1.txt');   % Malha fechada
mf3 = readtable('dados_parte2.txt');   % Malha aberta

tab1 = processa_dados_estufa(mf1,1);
tab2 = processa_dados_estufa(mf2,1);
tab3 = processa_dados_estufa(mf3,1);